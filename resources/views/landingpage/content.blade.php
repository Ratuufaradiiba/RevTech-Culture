...
<div class="col-lg-8 col-md-12 col-sm-12 col-xs-12">
	<article class="post-list mb-5 pb-4 border-bottom">
		...
		<a class="post-thumb mb-3 d-block" href="{{ url('culturedetail', $culture1->id)}}">
		...
		<div class=" meta-cat">
			<span class="letter-spacing cat-name font-extra text-uppercase font-sm">{{ $culture1->kategori->nama_kategori}}</span>
		</div>
		<h3 class="post-title mt-2"><a href="{{ url('culturedetail', $culture1->id)}}">{{ $culture1->nama_culture }}</a></h3>

		<div class="post-meta footer-meta">
			<ul class="list-inline">
				<li class="post-like list-inline-item">
					<span class="font-sm letter-spacing-1 text-uppercase"><i class="ti-time mr-2"></i>{{ $culture1->created_at->diffForHumans() }}</span>
				</li>
		...
		<div class="row no-gutters">
			...
			<div class="col-md-7">
				<div class="post-article mt-sm-3">
					<div class="meta-cat">
						<span class="letter-spacing cat-name font-extra text-uppercase font-sm">{{ $row->kategori->nama_kategori}}</span>
					</div>
					<h3 class="post-title mt-2"><a href="{{ url('culturedetail', $row->id)}}">{{ $row->nama_culture }}</a></h3>
